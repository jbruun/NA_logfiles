...
#Then load clickstreamClusterMethodSessionSimilarityNetwork.r
...
for (i in 1:length(overlap[,1])){
  memMat[overlap$node[i],overlap$group[i]]<-overlap$flow[i]
}
memMat<-memMat/rowSums(memMat) #normalized membership matrix
memMat[is.na(memMat)]<-0 # replace NAs with 0s
memMat[17,11]<-1
memMat[147,12]<-1

Fmemb<-colSums(memMat) #To be used in mean and standard deviaton calculations
#Network Measures (FRACTIONAL)
...
#Network Measures (FULL MEMBERSHIP)
meanNf<-vector()
confNf<-vector()
for (i in 1:10){
...
#Cluster Scores
####Scores weighted with membership weight#############
WmeanPCA1<-vector()
WconfPCA1<-vector()
for (i in 1:10){
...
sdMuOLD<-function(x,muX,memMat,i){
...
CIG<-c(CIA,CIB)
x<-c(1:2)
plot(x,meanMuG,ylim=range(c(meanMuG-CIG, meanMuG+CIG)),
     pch=19,  ylab="Mean mu +/- SD", xlab = "Cluster"
)
